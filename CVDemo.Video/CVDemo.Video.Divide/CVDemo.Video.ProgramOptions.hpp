	...
	namespace video
	{
		...
		public:
			...
			void DisplayInputOptions(std::ostream& os)
			{
				...
				if (vm.count("frame-sampling-interval")) {
					os << "Frame Sampling Interval is set to " << vm["frame-sampling-interval"].as<int>() << "." << std::endl;
				}

				if (vm.count("output-folder")) {
					os << "Output-folder is set to " << vm["output-folder"].as<std::string>() << "." << std::endl;
				}
		...
		public:
			...
			void DisplayFrameSamplingInterval(std::ostream& os)
			{
				if (vm.count("frame-sampling-interval")) {
					os << "Frame Sampling Interval is set to " << vm["frame-sampling-interval"].as<int>() << "." << std::endl;
				}
			}
			void DisplayOutputFolder(std::ostream& os)
			{
				if (vm.count("output-folder")) {
					os << "Output-folder is set to " << vm["output-folder"].as<std::string>() << "." << std::endl;
				}
		...
		public:
			...
			int GetFrameSamplingInterval()
			{
				...
				return vm["frame-sampling-interval"].as<int>();
			}
			std::string GetOutputFolder()
			{
				...
				return vm["output-folder"].as<std::string>();
			}
